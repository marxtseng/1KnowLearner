...
public class ExamFragment extends UnitFragment
    ...
    public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        mView = inflater.inflate(R.layout.fragment_unit_exam, container, false);
    ...
    protected void initFragment() {
        try {
            ...
            contentView.loadUrl(mUnit.getString("content_url"));

            JSONArray quizzes = new JSONArray();
            String[] contents = mUnit.getString("content").split("\n");

            for (int i = 0; i < contents.length; i++) {
                JSONArray options = new JSONArray();
                String[] quiz = contents[i].replaceAll("\t", ",").split(",");

                for (int j = 0; j < quiz.length; j++) {
                    quiz[j] = quiz[j].replaceAll("\"", "").replaceAll(" ", "");

                    if (j == 2) {
                        String[] ops = quiz[j].split("");
                        for (int k = 1; k < ops.length; k++) {
                            JSONObject jsonOption = new JSONObject();
                            jsonOption.put("item", ops[k]);
                            jsonOption.put("value", Math.pow(2, k-1));
                            jsonOption.put("correct", quiz[1].indexOf(ops[k]) > -1);
                ...
                jsonQuiz.put("uqid", "quiz" + (i+1));
                jsonQuiz.put("no", i+1);
                jsonQuiz.put("options", options);
                jsonQuiz.put("quiz_type", quiz[1].length() > 1 ? "multi" : "single");
                jsonQuiz.put("answer", quiz[1].length() == 1 ? quiz[1] : "");

                quizzes.put(jsonQuiz);
            }
